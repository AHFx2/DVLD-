...
namespace DVLD_Interface.Controls.ctrMISC
{
    public partial class ctrRenewLicense : UserControl
    {
        ...
        private void _FillInterNationalicesnApplicationInfo()
        {
            ...
            //application.ApplicationTypeID = Convert.ToByte(clsApplication.enApplicationType.NewIntrnationlLicense);
            //application.ApplicationStatus = 1;
            //application.LaststautDate = DateTime.Now;
            //application.ApplicationDate = DateTime.Now;
            //application.UserCreatIt = clsGloble.User.ID;
            //lblAppFees.Text = clsApplicationType.GetApplicationType(application.ApplicationTypeID).Fees.ToString();
            //application.PaidFees = Convert.ToInt16(lblAppFees.Text);
            //application.ApplicantPersonID = clsPerson.GetPerson(Convert.ToInt16(this.ctrFindLicense1.ctrLincesCard1.LicenseInformation.Rows[0]["PersonID"])).ID;
        ...
        private void _FillTheBoxInformation()
        {
            ...
            //lblAppFees.Text = clsApplicationType.GetApplicationType(Convert.ToByte(clsApplication.enApplicationType.NewIntrnationlLicense)).Fees.ToString();
            //lblDate.Text = DateTime.Now.ToShortDateString();
            //lblIssueDate.Text = DateTime.Now.ToShortDateString();
            //lblEpirtationDate.Text = DateTime.Now.AddYears(1).ToShortDateString();
            //lblAppFees.Text = clsApplicationType.GetApplicationType(Convert.ToByte(clsApplication.enApplicationType.RenewLicense)).Fees.ToString();
